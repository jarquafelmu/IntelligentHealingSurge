...
(function() {
	...
	const defines = Object.freeze({
		...
		/**
		 * A enumeration of the healing surge status
		 */
		healingSurgeStatus: {
			READY: 1,
			NOTREADY: 0
		...
		/**
		 * An enumberation of the attribute types that can be used for setting attributes
		 */
		attributeTypes: {
			CURRENT: "current",
			MAX: "max"
		...
		/**
		 * Easy to reference and customize list for the macros generated by the initialize command
		 */
		macroNames: {
			surge: "Healing-Surge",
			short: "Short-Rest",
	...
	const generateEmote = function() {
		return emoteOptions[randomInteger(defines.emoteOptions.length) - 1];
	}
	...
	var sendFeedback = function(msg, name) {
		...
		sendChat(defines.feedbackName, content);
	};
	...
	const templates = (function() {
		...
		const buildButton = function(label, command) {
			return `[${label}](!${defines.apiInvoke} -${command})`;
		};
		return {
	...
	class Character {
		...
		constructor(journalObj) {
			...
			this.hp = parseInt(this.getAttr(defines.friendlyAttributeNames.hp));

			this.maxHp = parseInt(this.getAttr(defines.friendlyAttributeNames.hp, defines.attributeTypes.MAX));

			this.hitDice = parseInt(this.getAttr(defines.friendlyAttributeNames.hitDice));

			this.maxHitDice = parseInt(this.getAttr(defines.friendlyAttributeNames.hitDice, defines.attributeTypes.MAX));

			this.hitDie = parseInt(this.getAttr(defines.friendlyAttributeNames.hitDie));

			this.healingSurge = this.getHealingSurgeValue();

			this.conMod = this.calculateModifier(parseInt(this.getAttr(defines.friendlyAttributeNames.conMod)));

			this.level = parseInt(this.getAttr(defines.friendlyAttributeNames.level));
		}
		...
		getHealingSurgeValue() {
			let retVal = 0;
			try {
				retVal = parseInt(this.getAttr(defines.friendlyAttributeNames.healingSurge));
			} catch (e) {
				if (e instanceof AttributeDoesNotExistException) {
					...
					this.setAttribute(defines.friendlyAttributeNames.healingSurge, defines.healingSurgeStatus.READY);
					retVal = defines.healingSurgeStatus.READY;
				} else {
		...
		getAttr(name, type) {
			if (type === undefined || type === null) {
				type = defines.attributeTypes.CURRENT;
			}
		...
		isHealingSurgeReady() {
			...
			return this.healingSurge === defines.healingSurgeStatus.READY;
		 ...
		 */
		updateHitDice() {
			this.setAttribute(defines.friendlyAttributeNames.hitDice, this.hitDice);
		}
		...
		updateHp(healAmount) {
			...
			this.setAttribute(defines.friendlyAttributeNames.hp, this.hp);
		}
		...
		doExhaustHealingSurge() {
			...
			this.setAttribute(defines.friendlyAttributeNames.healingSurge, defines.healingSurgeStatus.NOTREADY);
		}
	...
	const buildMacros = (function() {
		const create = function (name, command, playerid) {
			...
			if (!obj) {
				createObj("macro",
					{
						name: name,
						action: `!${defines.apiInvoke} -${command}`,
						visibleto: "all",
						istokenaction: true,
						playerid: playerid
					});
		...
		const run = function(playerid) {
			...
			create(defines.macroNames.surge, defines.commands.surge, playerid);
			create(defines.macroNames.short, defines.commands.shortRest, playerid);
			create(defines.macroNames.long, defines.commands.longRest, playerid);
		};
		return {
			run: run
		};
	...
	const handleInput = function(userInput) {
		...
		if (args.indexOf(`!${defines.apiInvoke}`, "") === 0) { // ensure that we are actually being called
			args = args.replace(`!${defines.apiInvoke}`, "").trim();

			try {
				if (args.length !== 0) {
					if (args.indexOf(`-${defines.commands.initialize}`) === 0) {
						...
						if (args.indexOf(`-${defines.commands.surge}`) === 0) {
							character.doHeal(sender);
						} else if (args.indexOf(`-${defines.commands.shortRest}`) === 0) {
							character.doShortRest(sender);
						} else if (args.indexOf(`-${defines.commands.longRest}`) === 0) {
							character.doLongRest(sender);
						} else if (args.indexOf(`-${defines.commands.exhaust}`) === 0) {
	...
	const ready = function() {
		log(`${defines.feedbackName} ready.`);
	};
